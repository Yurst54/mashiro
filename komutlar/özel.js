...
exports.run = async(bot, message, args) => {
                ...
                let mentionEmbed = new Discord.RichEmbed()
                     .setDescription(`Lütfen \`komut\` - \`gönderilecek şey\` olarak kullanınız.\nÖrnek: \`\`\`wp!özelkomutekle minecraft-ip play.serveripniz.com\`\`\``)
                     .setColor('RED')
                    // Return Statements
               var user = message.mentions.users.first() || message.author;
  let owner = await db.fetch(`${user.id}`);
  //let gold = require("./gold.json")
//if (!message.author.id(gold.gold7)) return message.channel.sendEmbed
  if (!message.member.hasPermission('MANAGE_GUILD') && message.author.id !== `${owner}`) return message.channel.sendEmbed(new Discord.RichEmbed().setDescription('Bu Komutu Kullanmak İçin `Sunucuyu Yönet` Yetkisi Lazım! \n Veya Gold uye Degilsin').setColor("RED"));
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['özelkomutekle','özelkomut-ekle','özel-komutekle','öke'],
  permLevel: 0
};

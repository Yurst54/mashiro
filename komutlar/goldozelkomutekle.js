...
message.channel.send("<a:shardr:666980258121121802>Bu Komutu Kullanabilmek İçin **Gold Üye** Olmalısın")
  }else{
      if( kullanıcı == 'gold'){        
       let prefix = await db.fetch(`guildPrefix_${message.guild.id}`) || "/";
                let mentionEmbed = new Discord.RichEmbed()
                     .setDescription(`Lütfen \`komut\` - \`gönderilecek şey\` olarak kullanınız.\nÖrnek: \`\`\`!!gold-komut-ekle minecraft-ip play.serveripniz.com    Sadece 1 Komut Ekleyebilirsiniz\`\`\``)
                      .setColor('8e0505')
                    // Return Statements
